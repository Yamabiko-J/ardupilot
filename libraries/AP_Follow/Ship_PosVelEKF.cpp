...
void Ship_PosVelEKF::init(float pos, float posVar, float vel, float velVar)
{
    _state[0] = pos;
    _state[1] = vel;
    _cov[0] = posVar;
...
void Ship_PosVelEKF::predict(float dt, float dVel, float dVelNoise)
{
...
void Ship_PosVelEKF::fusePos(float pos, float posVar)
{
    float newState[2];
    float newCov[3];
...
float Ship_PosVelEKF::getPosNIS(float pos, float posVar)
{
