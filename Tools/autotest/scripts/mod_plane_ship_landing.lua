...
function update()
   ...
   -- gcs:send_text(MAV_SEVERITY.INFO, string.format("target_alt %.2f", get_target_alt()))
   -- gcs:send_text(MAV_SEVERITY.INFO, string.format("target_pos %.2f", target_pos:alt()))

   logger.write('TLND','tgalt','f', get_target_alt())
   local temp_tp = target_pos:copy()
   if vehicle_mode ~= MODE_QRTL then
      temp_tp:alt(0)
   end
   ahrs:set_home(temp_tp)
